import React, { useEffect, useState } from "react";
import axios from "axios";
import RefreshModal from "../modals/RefreshModal";
import Loading from "../components/Loading";
import PreviewSideBar from "../components/PreviewSideBar";
import PrevLoadingSideBar from "../components/PrevLoadingSideBar";
import { useSearchParams } from "react-router-dom";
import { useNavigate } from "react-router-dom";
import { FastAverageColor } from "fast-average-color";
import TaPrevBody from "../components/TaPrevBody";
import styled from "styled-components";
import SmallScreen from "../components/SmallScreen";
const TaPrev = ({ Auth }) => {
  const [domColor, setDomColor] = useState("white");
  const [imgPrev, setImgPrev] = useState("");
  const [rotateLogo, setRotateLogo] = useState(false);
  const [light, setLight] = useState(false);
  const [title, setTitle] = useState("title");
  const [order, setOrder] = useState(0);
  const [popularity, setPopularity] = useState(0);
  const [genre, setGenre] = useState("genre");
  const [almap, setAlmap] = useState({});
  const [user, setUser] = useState({});
  const [taLikes, setTaLikes] = useState([]);
  // eslint-disable-next-line
  const [year, setYear] = useState("****");
  // eslint-disable-next-line
  const [artist, setArtist] = useState("artist");
  const [topArtists, setTopArtists] = useState([
    {
      title: "Asmahan",
      genre: "CLASSIC ARAB POP",
      img: "https://i.scdn.co/image/ab67616d0000b2735e386e1555ec2cc9b24b2642",
      popularity: 37,
      preview_url:
        "https://p.scdn.co/mp3-preview/02989db0228cb3df8dc7eb7ff22984e7803f5dcc?cid=5bb9526f55034e9da5b76af815696e62",
      id: "2vFXOKUQ2uhI9D8vKozoK6",
    },
    {
      title: "Eminem",
      popularity: 88,
      genre: "DETROIT HIP HOP",
      img: "https://i.scdn.co/image/ab67616d0000b2736ca5c90113b30c3c43ffb8f4",
      preview_url:
        "https://p.scdn.co/mp3-preview/8625034728d0e1a6971ab17f003fecaad74f63b4?cid=5bb9526f55034e9da5b76af815696e62",
      id: "7dGJo4pcD2V6oG8kP0tJRR",
    },
  ]);
  // eslint-disable-next-line
  const [taIds, setTaIds] = useState([
    "2vFXOKUQ2uhI9D8vKozoK6",
    "7dGJo4pcD2V6oG8kP0tJRR",
  ]);
  const [tasortOpt, setTaSortOpt] = useState("Affinity");
  const [showModal, setShowModal] = useState(false);
  const [loading, setLoading] = useState(true);
  // eslint-disable-next-line
  const [searchParams, setSearchParams] = useSearchParams();
  const navigate = useNavigate();
  const [meta, setMeta] = useState({
    user_name: "Ahmed Ghonim",
    date_month: "Apr",
    date_day: 18,
  });
  const fac = new FastAverageColor();

  document.getElementById("myHtml").style.backgroundColor = domColor;

  /* loading period*/
  setTimeout(() => {
    setLoading(false);
  }, 2000);

  // Get the current user
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const responce = await axios.get("https://api.spotify.com/v1/me", {
          headers: {
            Authorization: `Bearer ${window.localStorage.getItem("token")}`,
          },
        });
        setUser(responce.data);
      } catch (err) {
        setShowModal(true);
      }
    };
    if (window.localStorage.getItem("token")) fetchUser();
  }, []);

  /*fetching preview topArtists from mongoDB Atlas*/
  useEffect(() => {
    const fetchArtists = async () => {
      try {
        let ids = [];
        const res = await axios({
          method: "get",
          url: `https://eu-west-1.aws.data.mongodb-api.com/app/application-0-uvvbg/endpoint/Getcards?id=${searchParams.get(
            "id"
          )}`,
          headers: {},
        });
        if (res.data) {
          for (let i = 0; i < res.data.cards.length; i++) {
            ids.push(res.data.cards[i].id);
          }
          setTopArtists(res.data.cards);
          setMeta(res.data.meta);
          setTaIds(ids);
          localStorage.setItem(
            "prevTopArtists",
            JSON.stringify(res.data.cards)
          );
        } else {
          navigate("/notFound");
        }
        //console.log(res);
      } catch (err) {
        console.log(err);
        navigate("/notFound");
      }
    };
    fetchArtists();
    // eslint-disable-next-line
  }, []);

  useEffect(() => {
    const intiatePrev = () => {
      setImgPrev(topArtists[0].img);
      setTitle(topArtists[0].title);
      setOrder(1);
      setGenre(topArtists[0].genre);
      setPopularity(topArtists[0].popularity);
      setArtist(null);
      setYear(null);
      fac
        .getColorAsync(topArtists[0].img, {
          algorithm: "dominant",
        })
        .then((color) => {
          setDomColor(color.hex);
          color.isLight ? setLight(true) : setLight(false);
        });
    };
    if (topArtists.length > 0) intiatePrev();
    // eslint-disable-next-line
  }, [topArtists]);

  /*Checking what the current user follow from the preview artists */

  useEffect(() => {
    let ids = "";
    for (let i = 0; i < taIds.length; i++) {
      ids += topArtists[i].id;
      if (i < topArtists.length - 1) ids += ",";
    }
    const fetchTaFollow = async () => {
      try {
        const responce = await axios.get(
          `https://api.spotify.com/v1/me/following/contains?type=artist&&ids=${ids}`,
          {
            headers: {
              Authorization: `Bearer ${window.localStorage.getItem("token")}`,
            },
          }
        );
        setTaLikes(responce.data);
      } catch (err) {}
    };
    if (window.localStorage.getItem("token")) fetchTaFollow();
    // eslint-disable-next-line
  }, [taIds]);

  /*make the artist-liked map*/
  useEffect(() => {
    const obj = {};
    taIds.forEach((element, index) => {
      obj[element] = taLikes[index];
    });
    setAlmap(obj);
    //eslint-disable-next-line
  }, [taLikes]);

  return (
    <>
      <Wrap>
        {loading && <Loading LSB={<PrevLoadingSideBar />} />}

        {!loading && showModal ? (
          <RefreshModal
            Auth={Auth}
            onClose={() => {
              setShowModal(false);
            }}
          ></RefreshModal>
        ) : null}

        {!loading ? (
          <>
            <div
              style={{
                width: "100%",
                height: "auto",
                display: "flex",
                backgroundColor: `${domColor}`,
              }}
            >
              <PreviewSideBar
                imgPrev={imgPrev}
                rotateLogo={rotateLogo}
                light={light}
                title={title}
                order={order}
                year={null}
                genre={genre}
                popularity={popularity}
                artist={null}
                meta={meta}
                type={"Top Artists"}
              ></PreviewSideBar>
              <TaPrevBody
                setDomColor={setDomColor}
                setImgPrev={setImgPrev}
                setRotateLogo={setRotateLogo}
                setLight={setLight}
                setTitle={setTitle}
                setOrder={setOrder}
                setYear={setYear}
                setGenre={setGenre}
                setPopularity={setPopularity}
                setArtist={setArtist}
                topArtists={topArtists}
                setTopArtists={setTopArtists}
                almap={almap}
                setShowModal={setShowModal}
                setAlmap={setAlmap}
                user={user}
                tasortOpt={tasortOpt}
                setTaSortOpt={setTaSortOpt}
                meta={meta}
              ></TaPrevBody>
            </div>
          </>
        ) : null}
      </Wrap>
      <SmallScreen></SmallScreen>
    </>
  );
};

export default TaPrev;

const Wrap = styled.div`
  @media (max-width: 1100px) {
    display: none;
  }
`;
