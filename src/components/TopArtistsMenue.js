...
const TopArtistsMenue = ({
  ...
  setImgPrev(topArtists[0].images[0].url);
    ...
    setGenre(
      topArtists[0].genres[0]?.toUpperCase() ||
        "not yet classified".toUpperCase()
    );
  ...
  */
  ...
  const handleShare = () => {
    const defaultTopArtists = JSON.parse(localStorage.getItem("topArtists"));
    const DateArr = new Date().toString().split(" ");
    const artistsArr = [];
    const meta = {};
    for (let i = 0; i < defaultTopArtists.length; i++) {
      const obj = {};
      obj["title"] = defaultTopArtists[i].name;
      obj["popularity"] = defaultTopArtists[i].popularity;
      obj["genre"] =
        defaultTopArtists[i].genres[0]?.toUpperCase() ||
        "not classified yet".toUpperCase();
      obj["img"] = defaultTopArtists[i].images[0].url;
      obj["preview_url"] = apmap[defaultTopArtists[i].id];
      obj["id"] = defaultTopArtists[i].id;
      artistsArr.push(obj);
    }
    ...
    const PostDb = async () => {
      try {
        ...
        await navigator.clipboard.writeText(
          `https://sharify-gh.netlify.app/TopArtists?id=${res.data.insertedId}`
        );
  ...
  useEffect(() => {
    const resetMenueColor = () => {
      fac
        .getColorAsync(topArtists[3].images[0].url, {
          algorithm: "dominant",
        })
        .then((color) => {
  ...
  const sortAff = () => {
    ...
    setTopArtists(JSON.parse(localStorage.getItem("topArtists")));
  };
...
export default TopArtistsMenue;
