...
const TopTracksMenue = ({
  ...
  const handleShare = () => {
    const defaultTopTracks = JSON.parse(localStorage.getItem("topTracks"));
    const DateArr = new Date().toString().split(" ");
    const songsArr = [];
    const meta = {};
    for (let i = 0; i < defaultTopTracks.length; i++) {
      const obj = {};
      obj["title"] = defaultTopTracks[i].name;
      obj["artist"] = defaultTopTracks[i].artists[0].name;
      obj["year"] = defaultTopTracks[i].album.release_date.substring(0, 4);
      obj["popularity"] = defaultTopTracks[i].popularity;
      obj["genre"] =
        tgmap[defaultTopTracks[i].artists[0].id]?.toUpperCase() ||
        "not classified yet".toUpperCase();
      obj["img"] = defaultTopTracks[i].album.images[0].url;
      obj["preview_url"] = defaultTopTracks[i].preview_url;
      obj["id"] = defaultTopTracks[i].id;
      songsArr.push(obj);
    }
    ...
    const PostDb = async () => {
      try {
        ...
        await navigator.clipboard.writeText(
          `https://sharify-gh.netlify.app/TopTracks?id=${res.data.insertedId}`
        );
  ...
  const handleSave = async () => {
    ...
    try {
      const { data } = await axios({
        ...
        data: {
          name: `${DateArr[1]} - ${DateArr[2]} ${user.display_name}'s Top Tracks`,
          description: `${user.display_name}'s top monthly tracks as recorded at ${DateArr[1]} - ${DateArr[2]} by Sharify Web app (Sorted by ${ttsortOpt})`,
          public: false,
        },
      });
  ...
  useEffect(() => {
    const resetMenueColor = () => {
      fac
        .getColorAsync(topTracks[3].album.images[0].url, {
          algorithm: "dominant",
        })
        .then((color) => {
  ...
  const sortDateA = () => {
    ...
    temp.sort((a, b) => {
      return (
        a.album.release_date.substring(0, 4) -
        b.album.release_date.substring(0, 4)
      );
    });
    setTopTracks(temp);
    resetPrev();
